...
async def create_order(order: Order, exchange: Exchange = Depends(get_exchange),
...
def get_order(order_id: UUID, db: InMemoryDatabase = Depends(get_db)):
    """
...
def cancel_order(order_id: UUID, db: InMemoryDatabase = Depends(get_db)):
    """
...
def get_order_status(order_id: UUID, db: InMemoryDatabase = Depends(get_db)):
    """
...
def get_active_orders(user_id: UUID, db: InMemoryDatabase = Depends(get_db)):
    """
...
async def get_orderbook(symbol: str, exchange: Exchange = Depends(get_exchange)):  # Make async
    """
